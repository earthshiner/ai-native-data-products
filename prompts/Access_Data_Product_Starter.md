...
-- Every AI-Native Data Product creates {Product}_ROLE_AGENT for agent access.
-- Confirm it was granted before attempting module queries.
SELECT RoleName
FROM DBC.RoleInfoV
...
**If you receive `Error 3523` (no SELECT access):** The Access Layer may not have been deployed yet, or your service account has not been granted `{Product}_ROLE_AGENT`. Do not attempt to work around this by requesting direct grants to individual databases. Ask the data product owner to run Phase 1.5 and Phase 2.5 of the Access Layer DCL.

**If you need to generate DDL (not just query):** Before writing any CREATE statement, locate the organisation's Object Placement Standard implementation. It specifies which database each object type belongs in. Never assume co-location or invent database names. If no implementation can be found at the locations listed in the Object Placement Standard Spec, stop and ask.
